...
| **redundant_mistakes.md** | Root-level repo memory for repeated mistakes and regressions. Read before new edits or slices. Tracks UI contrast mistakes (for example light buttons with unreadable idle text), backend/data pitfalls (circular imports, SQLite foreign keys, duplicate joinedload patterns, id reuse/orphans), and environment/validation mistakes that have already occurred in this project. |
